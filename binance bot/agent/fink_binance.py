...
data_source = []
data_source.append({'key': 'o7F2kS6QGIzbMm73iTXmwXlDqbAV4bIHFJcKsPw6dMf447nk7okcRVyprbT8t2Fu', 'secret': '8bzSZklysXWhv4yxU9RKGlVbwmuZuzm0o1VaqjfIs4eZLKGmCeNY3XdSlAeHFj1k'})  
data_source.append({'key': 'XFdh1sK860gKQMUGLEEIcny04qqOIxh9Zim4BNdwXWMzCxkeBAH5V3Lz23RCqZTX', 'secret': 'ic1IEHFdwwBS3LV9z1BwtQfOqZszxZdrlZqqhtJNySfpCGGTNocIO1PdBWm8C9uV'})     
...
class trading_account(object):
    ...
    def __init__(self,key,secret):
        ...
        self.data_source = Client(self.data_api_key, self.data_api_secret)
        print("*******************************************")
    ...
    def get_params(self):
        ...
        parser.add_argument('-c', '--currency',
                        ...
                        help='The currency to be traded')
        parser.add_argument('-b', '--base',
                        action='store',  # tell to store a value
                        dest='base',  # use `pair` to access value
                        help='The base currency')
        parser.add_argument('-w', '--weight',
                        action='store',  # tell to store a value
                        dest='weight',  # use `pair` to access value
                        help='The trading weight')
        ...
        self.currency = str(action.currency).upper()
        self.base = str(action.base).upper() 
        self.agent_weight = float(action.weight) ##how much the agent is using for the order in base
    ...
    def get_historical_data(self):
        ...
        while True:
            try:
                result = self.data_source.get_historical_klines(self.symbol, Client.KLINE_INTERVAL_12HOUR, "1 Jan, 2017") ##Get 12 Hours in Data)
                index = 0
                for i in result:
...
def main():
    ...
    target = trading_account('o7F2kS6QGIzbMm73iTXmwXlDqbAV4bIHFJcKsPw6dMf447nk7okcRVyprbT8t2Fu','8bzSZklysXWhv4yxU9RKGlVbwmuZuzm0o1VaqjfIs4eZLKGmCeNY3XdSlAeHFj1k')    
    target.trade()
