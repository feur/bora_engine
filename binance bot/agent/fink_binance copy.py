...
class trading_account(object):
    ...
    def get_balance(self):
        ...
        time.sleep(2) ## 2 second sleep to avoid API~~~~~
    ...
    def MakeBuyOrder(self,order_block):
       ...
       # print("")
        ...
        if self.balance_base >= self.agent_weight: 
    ...
    def get_historical_trades(self):
        ...
        hist = client.get_historical_trades(symbol=self.symbol)
        
        ##need to go through the history and find the sum of orders consumed so far 
        #total_buy
        #otla_sell 
    ...
    def Get_Entry_Position(self, entry_block,rl):
        ...
        if entry_block.quantity_base > self.balance_base and self.support_needed == 1:   
            print("<<<<<<<<<<<<<<BALANCE NOT ENOUGH>>>>>>>>>>")
            self.log_activities("","")
            return 0
        ...
        self.log_activities(str(entry_block.buy_price),"")
    ...
    def GetExitPosition(self):
        ...
        if isinstance(exit_order, Block): 
            ...
            if d > (1 + self.exchange_fee):
                ...
                print("Distance insufficient....Possibly supporting with a new Entry")
                self.log_activities("","")
    ...
    def trade(self):
        ...
        while True:      
            ...
            if len(self.back_orders) != 0:     
                ...
                if self.back_orders[-1].sell_uid != 0:
                    ...
                    if str(status['status']) == "NEW" or str(status['status']) == "PARTIALLY_FILLED":  ##we look to update sell order     
                        ...
                        if (self.base == "USDT" and self.back_orders[-1].quantity_base > self.base_usd_min) or (self.base == "BTC" and self.balance_base_holding > self.base_btc_min):
                ...
                ##check if we have a buy order registered
                    ...
                    if str(status['status']) == "NEW":          
                        ...
                        if isinstance(buy_pos, Block) and buy_pos.buy_price != self.back_orders[-1].buy_price: ##we get a different exit position
                            ...
                            print("")
                        elif buy_pos < 0:                                 ##we don't needa  suppport block anymore
                            order_state = self.CancelOrder(self.back_orders[-1].buy_uid) ##cancel the current order
            ...
            print("")
            print("Total Unit Holding: ", self.balance_unit , self.currency, "or ", self.balance_base_holding, self.base,"..... Total Base Balance ", self.balance_base,self.base,"..... Total HOLDING ",self.total_base_balance,self.base)
...
def main():
    ...
    target_xrp = trading_account('XRP','USDT','o7F2kS6QGIzbMm73iTXmwXlDqbAV4bIHFJcKsPw6dMf447nk7okcRVyprbT8t2Fu','8bzSZklysXWhv4yxU9RKGlVbwmuZuzm0o1VaqjfIs4eZLKGmCeNY3XdSlAeHFj1k',250)    
    target_xrp.trade()
